                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">
                      {formData.event_type === 'refund_requested' ? 'Motivo do Reembolso' : 'Sub-condição (Opcional)'}
                      <span className="text-gray-400 font-normal ml-2 text-xs" title="Se preenchido, este script só aparecerá se a palavra selecionada estiver contida no motivo do estorno ou recusa.">?</span>
                    </label>
                    {formData.event_type === 'refund_requested' ? (
                      <select
                        value={formData.sub_condition || ''}
                        onChange={e => setFormData({...formData, sub_condition: e.target.value})}
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 font-medium bg-white"
                      >
                        <option value="">Qualquer Motivo / Padrão</option>
                        <option value="acessar">Dificuldade de Acesso</option>
                        <option value="qualidade">Qualidade do Produto</option>
                        <option value="arrepend">Arrependimento / Desistência</option>
                      </select>
                    ) : (
                      <input 
                        type="text" 
