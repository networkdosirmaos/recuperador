...
export function InboxView({ targetUserId, viewerRole, viewerId }: InboxViewProps) {
  ...
  const { data: countsData, isLoading: loadingCounts } = useQuery({
    queryKey: ['seller_lead_counts', targetUserId],
    queryFn: () => sellerService.getLeadCounts(targetUserId),
    enabled: !!targetUserId,
    refetchInterval: 30000 // Apenas os contadores atualizam levemente a cada 30s
  })

  const counts = countsData || { pendentes: 0, em_andamento: 0, finalizados: 0, geladeira: 0, todos_ativos: 0 }
  ...
  const { isAnimating } = useLeadGamification(counts.pendentes || 0, counts.finalizados || 0)
  ...
  const selectedLeadData = filteredLeads.find((l: MyLead) => l.id === selectedLeadId) || null
